...
def train(conf, train_data_list, val_data_list):
    ...
    # create models
    network = model_def.Network(conf.feat_dim, conf.task_feat_dim, conf.cp_feat_dim, conf.dir_feat_dim, topk=conf.topk, task_input_dim=task_input_dim)
...
def forward(batch, data_features, network, conf, \
    ...
    if conf.model_version == 'model_aff_sec':
        pred_score = network.forward(pcs, task, ctpt1, ctpt2, dir1)

        # get groundtruth
        with torch.no_grad():
            recon_dir2 = actor.actor_sample_n(pcs, task, ctpt1, ctpt2, dir1, rvs=conf.rv_cnt).contiguous().view(batch_size * conf.rv_cnt, 6)
            gt_scores = critic.forward_n(pcs, task, ctpt1, ctpt2, dir1, recon_dir2, rvs=conf.rv_cnt)  # dir1: B*6; dir2: (B*rvs) * 6
            # print('pred_scores: ', gt_scores.shape)
            gt_score = gt_scores.view(batch_size, conf.rv_cnt, 1).topk(k=conf.topk, dim=1)[0].mean(dim=1)
            # print('gt_scores: ', gt_score.shape)
        loss = network.get_loss(pred_score.view(batch_size, -1), gt_score.view(batch_size, -1))


    elif conf.model_version == 'model_aff_fir':
        pred_score = network.forward(pcs, task, ctpt1)

        # get groundtruth
        with torch.no_grad():
            recon_dir1 = actor.actor_sample_n(pcs, task, ctpt1, rvs=conf.rv_cnt).contiguous().view(batch_size * conf.rv_cnt, 6)
            gt_scores = critic.forward_n(pcs, task, ctpt1, recon_dir1, rvs=conf.rv_cnt)  # dir1: B*6; dir2: (B*rvs) * 6
            gt_score = gt_scores.view(batch_size, conf.rv_cnt, 1).topk(k=conf.topk, dim=1)[0].mean(dim=1)
        loss = network.get_loss(pred_score.view(batch_size, -1), gt_score.view(batch_size, -1))
...
if __name__ == '__main__':
    ...
    for data_dir in offline_data_dir_list:
        train_data_list.extend(utils.append_data_list(data_dir, only_true_data=conf.only_true_data, primact_type=conf.primact_type))
    ...
    for data_dir in val_data_dir_list:
        val_data_list.extend(utils.append_data_list(data_dir, only_true_data=conf.only_true_data, primact_type=conf.primact_type))
