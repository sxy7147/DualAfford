"""
    ...
    Upgraded to support loading multiple primact_types:
        Keep separate true_data/false_data queues for each primact_type
        Make sure sampling equally from all types when get_item
"""
...
class SAPIENVisionDataset(data.Dataset):
    ...
    def __init__(self, primact_types, category_types, data_features, buffer_max_num, \
            abs_thres=0.01, rel_thres=0.5, dp_thres=0.5, img_size=224, no_true_false_equal=False):
    ...
    def __getitem__(self, index):
        ...
        for feat in self.data_features:
            ...
            elif feat == 'gripper_quaternion':
                dir1 = gripper_direction_camera
                dir2 = gripper_forward_direction_camera
                dir3 = np.cross(dir1, dir2)
                rotmat = np.vstack([dir1, dir2, dir3]).T
                q = Quaternion(matrix=rotmat)
                quat = np.array([q.w, q.x, q.y, q.z], dtype=np.float32)
                out = torch.from_numpy(quat).unsqueeze(0)
                data_feats = data_feats + (out,)
            ...
            elif feat == 'primact_one_hot':
                out = torch.zeros(1, len(self.primact_types)).float()
                out[0, self.primact_types.index(primact_type)] = 1
                data_feats = data_feats + (out,)
