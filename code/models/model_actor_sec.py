...
class Network(nn.Module):
    def __init__(self, feat_dim, task_feat_dim, cp_feat_dim, dir_feat_dim, z_dim=128, lbd_kl=1.0, lbd_dir=1.0, task_input_dim=3):
        ...
        self.all_encoder = ActorEncoder(input_dim=feat_dim * 2 + task_feat_dim + cp_feat_dim * 2 + dir_feat_dim * 2, output_dim=z_dim)
        self.decoder = ActorDecoder(input_dim=feat_dim * 2 + task_feat_dim + cp_feat_dim * 2 + dir_feat_dim + z_dim, output_dim=6)
    ...
    def forward(self, pcs, task, cp1, cp2, dir1, dir2):
        ...
        z_all, mu, logvar = self.all_encoder([net1, net2, task_feats, cp1_feats, cp2_feats, dir1_feats, dir2_feats])
        recon_dir2 = self.decoder([net1, net2, task_feats, cp1_feats, cp2_feats, dir1_feats, z_all])

        return recon_dir2, mu, logvar


    def get_loss(self, pcs, task, cp1, cp2, dir1, dir2):
        batch_size = pcs.shape[0]
        recon_dir2, mu, logvar = self.forward(pcs, task, cp1, cp2, dir1, dir2)
    ...
    def actor_sample(self, pcs, task, cp1, cp2, dir1):
        ...
        recon_dir2 = self.decoder([net1, net2, task_feats, cp1_feats, cp2_feats, dir1_feats, z_all])
        recon_dir2 = recon_dir2.reshape(-1, 2, 3).permute(0, 2, 1)
        recon_dir2 = self.bgs(recon_dir2)
        recon_dir2 = recon_dir2.permute(0, 2, 1)
        recon_dir2 = recon_dir2[:, :2, :]

        return recon_dir2


    def actor_sample_n(self, pcs, task, cp1, cp2, dir1, rvs=100):
        batch_size = pcs.shape[0]
        ...
        recon_dir2 = self.decoder([expanded_net1, expanded_net2, expanded_task_feats,
                                expanded_cp1_feats, expanded_cp2_feats, expanded_dir1_feats, expanded_rvs])
        recon_dir2 = recon_dir2.reshape(-1, 2, 3).permute(0, 2, 1)
        recon_dir2 = self.bgs(recon_dir2)
        recon_dir2 = recon_dir2.permute(0, 2, 1)
        recon_dir2 = recon_dir2[:, :2, :]
    ...
    def actor_sample_n_diffCtpts(self, pcs, task, cp1, cp2, dir1, rvs_ctpt=10, rvs=10):
        batch_size = pcs.shape[0]
        ...
        z_all = torch.Tensor(torch.randn(batch_size * rvs_ctpt, rvs, self.z_dim)).to(net1.device)

        expanded_rvs = z_all.reshape(batch_size * rvs_ctpt * rvs, -1)       # (B * rvs_ctpt * rvs) * dim
        ...
        expanded_dir1_feats = dir1_feats.unsqueeze(dim=1).repeat(1, rvs_ctpt * rvs, 1).reshape(batch_size * rvs_ctpt * rvs, -1)
        expanded_cp2_feats = cp2_feats.unsqueeze(dim=1).repeat(1, rvs, 1).reshape(batch_size * rvs_ctpt * rvs, -1)

        recon_dir2 = self.decoder([expanded_net1, expanded_net2, expanded_task_feats,                               # (B * rvs_ctpt * rvs, 6)
                                   expanded_cp1_feats, expanded_cp2_feats, expanded_dir1_feats, expanded_rvs])
        recon_dir2 = recon_dir2.reshape(-1, 2, 3).permute(0, 2, 1)
        recon_dir2 = self.bgs(recon_dir2)
        recon_dir2 = recon_dir2.permute(0, 2, 1)
    ...
    def actor_sample_n_finetune(self, pcs, task, cp1, cp2, dir1, rvs_ctpt=100, rvs=100):
        batch_size = pcs.shape[0]
        ...
        expanded_cp2_feats = cp2_feats.unsqueeze(dim=1).repeat(1, rvs, 1).reshape(batch_size * rvs_ctpt * rvs, -1)
        expanded_dir1_feats = dir1_feats.unsqueeze(dim=1).repeat(1, rvs, 1).reshape(batch_size * rvs_ctpt * rvs, -1)
