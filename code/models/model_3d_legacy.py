...
class Network(nn.Module):
    ...
    def inference_actor(self, pcs):
        pcs = pcs.repeat(1, 1, 2)               # B x N x 6
        batch_size = pcs.shape[0]

        whole_feats = self.pointnet2(pcs)       # B x F x N
        net = whole_feats[:, :, 0]              # B x F

        rvs = torch.randn(batch_size, self.rv_cnt, self.rv_dim).float().to(net.device)                          # B x T x f
        expanded_net = net.unsqueeze(dim=1).repeat(1, self.rv_cnt, 1).reshape(batch_size * self.rv_cnt, -1)     # (B x T) x F
        expanded_rvs = rvs.reshape(batch_size * self.rv_cnt, -1)                                                # (B x T) x f
        expanded_pred_s6d = self.actor(expanded_net, expanded_rvs)
        pred_s6d = expanded_pred_s6d.reshape(batch_size, self.rv_cnt, 6)        # B x T x 6
        return pred_s6d
    ...
    def inference_critic(self, pcs, dirs1, dirs2, abs_val=False):
        ...
        whole_feats = self.pointnet2(pcs)
        net = whole_feats[:, :, 0]
        # print(net.shape, dirs1.shape, dirs2.shape)
